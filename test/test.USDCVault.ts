...
describe('USDC_Vault',async() =>{
    ...
    beforeEach(async () => {
        ...
        await usdy.connect(owner).setOperator(usdcvault.address);
    })

    describe("USDCVault: Deposit And Withdraw ",async () => {

    it("Deposit in USDCVault",async()=>{
        ...
        const USDCs = await new Receipt__factory(owner).attach(await usdcvault.USDCs());

        await whitelist.connect(owner).addWhiteList([owner.address,usdcvault.address,liquidityProvider.address,tokenPair]);
        ...
        await usdc.connect(owner).mint(owner.address,expandTo18Decimals(100000));
        ...
        await usdc.connect(owner).approve(liquidityProvider.address,expandTo18Decimals(100000));
        ...
        await liquidityProvider.connect(owner).addLiquidity(busd.address,usdc.address,expandTo18Decimals(10000),expandTo18Decimals(10000),expandTo18Decimals(1),expandTo18Decimals(1),owner.address);
        await usdc.connect(owner).approve(usdcvault.address,expandTo18Decimals(10000));

        await usdcvault.connect(owner).deposit(owner.address, expandTo18Decimals(1000), false);
        expect (await USDCs.balanceOf(usdcvault.address)).to.equal(expandTo18Decimals(1000));
        

    })
    it("Withdraw in USDCVault", async()=>{
        let tokenPair = await factory.getPair(usdy.address, busd.address);
        await admin.setUSDyBUSD(tokenPair);
        const pair_instance = await new UniswapV2Pair__factory(owner).attach(tokenPair); 
        const USDCs = await new Receipt__factory(owner).attach(await usdcvault.USDCs());
        ...
        await usdc.connect(owner).approve(usdcvault.address,expandTo18Decimals(10000));
        await usdcvault.connect(owner).setEpoch(10);
        await usdcvault.connect(owner).deposit(owner.address, expandTo18Decimals(1000), false);
        await mineBlocks(ethers.provider,90);
        await usdcvault.connect(owner).withdraw(owner.address,expandTo18Decimals(200),owner.address);
    ...
    describe("USDCVault : ClaimReobaseReward", async()=> {
        it("ClaimReobaseReward in USDCVault", async () => {
            let tokenPair = await factory.getPair(usdy.address, busd.address);
        await admin.setUSDyBUSD(tokenPair);
        ...
        let x = await usdcvault.connect(owner).rewards(owner.address);
        await usdcvault.connect(owner).claimReward(owner.address);
        })
    })
