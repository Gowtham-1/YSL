...
{ Admin, Admin__factory, 
    ...
    YSL, YSL__factory, BUSDVault, BUSDVault__factory, USDCVault, USDCVault__factory, USDTVault, USDTVault__factory, BSHARE, BSHARE__factory } 
    from "../typechain";

    import exp from "constants";

describe('DAI_Vault',async() =>{
    ...
    beforeEach(async () => {
        ...
        await usdy.connect(owner).setOperator(daiVault.address);
    })

    describe("DAIVault: Deposit And Withdraw ",async () => {

    it("Deposit in DAIVault",async()=>{
        ...
        const daiS = await new Receipt__factory(owner).attach(await daiVault.DAIs());

        await whitelist.connect(owner).addWhiteList([owner.address,daiVault.address,liquidityProvider.address,tokenPair]);
        ...
        await dai.connect(owner).mint(owner.address,expandTo18Decimals(100000));
        ...
        await dai.connect(owner).approve(liquidityProvider.address,expandTo18Decimals(100000));
        ...
        await liquidityProvider.connect(owner).addLiquidity(busd.address,dai.address,expandTo18Decimals(10000),expandTo18Decimals(10000),expandTo18Decimals(1),expandTo18Decimals(1),owner.address);
        await dai.connect(owner).approve(daiVault.address,expandTo18Decimals(10000));

        await daiVault.connect(owner).deposit(owner.address, expandTo18Decimals(1000), false);
        expect (await daiS.balanceOf(daiVault.address)).to.equal(expandTo18Decimals(1000));
        

    })
    it("Withdraw in DaiVault", async()=>{
        let tokenPair = await factory.getPair(usdy.address, busd.address);
        await admin.setUSDyBUSD(tokenPair);
        const pair_instance = await new UniswapV2Pair__factory(owner).attach(tokenPair); 
        const daiS = await new Receipt__factory(owner).attach(await daiVault.DAIs());
        ...
        await dai.connect(owner).approve(daiVault.address,expandTo18Decimals(10000));
        await daiVault.connect(owner).setEpoch(10);
        await daiVault.connect(owner).deposit(owner.address, expandTo18Decimals(1000), false);
        await mineBlocks(ethers.provider,90);
        await daiVault.connect(owner).withdraw(owner.address,expandTo18Decimals(200),owner.address);
    ...
    describe("DAIVault : ClaimReobaseReward", async()=> {
        it("ClaimReobaseReward in DaiVault", async () => {
            let tokenPair = await factory.getPair(usdy.address, busd.address);
        await admin.setUSDyBUSD(tokenPair);
        ...
        await whitelist.connect(owner).addWhiteList([owner.address,daiVault.address,liquidityProvider.address,tokenPair,usdyVault.address,yslBusdVault.address,xYslBusdVault.address,usdyBusdVault.address,bShareBusdVault.address,busdVault.address,daiVault.address,usdcvault.address,usdtvault.address]);
        ...
        let x = await daiVault.connect(owner).rewards(owner.address);
        await daiVault.connect(owner).claimReward(owner.address);
        })
    })
