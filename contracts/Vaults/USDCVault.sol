...
/**
@dev This contract is for USDC vault.
 */
contract USDCVault is Initializable, ReentrancyGuard, Pausable {
    ...
    address public USDCs; //Reciept token address
    ...
    function initialize(address _admin, address _USDC) external initializer {
        rewardRate = 10; // cofficient by 100
        slippage = 100; // cofficient by 100
        Admin = IAdmin(_admin);
        router = Admin.ApeswapRouter();
        USDC = _USDC;
        USDCs = Clones.clone(Admin.masterNTT());
        IReceipt(USDCs).initialize(_admin, address(this), "USDCs", "USDCs");
        epoch = 8 hours;
    ...
    */
    ...
    function deposit(address user, uint256 amount, bool isBUSD) external nonReentrant _completePause whenNotPaused() _securityCheck(user,user){
        require(amount > 0, "USDCVault: Amount must be greater than zero");
         if(rewards(user)[0] > 0){
        ...
        _convertBUSDToLP(
            ...
            Admin.USDy(),
            Admin.BUSD());
        IReceipt(USDCs).mint(address(this), amount);
    ...
    function withdraw(address user, uint256 amount, address sendTo) external whenNotPaused {
        require(user != address(0), "USDCvault: User address can not be empty");
        require(amount > 0, "USDCVault: Amount must be greater than zero");
    ...
    function distribute(uint256 _rebaseReward) external nonReentrant{
        ...
        if(IERC20(USDCs).totalSupply() != 0){
            currentReward = currentReward + ((_rebaseReward * 10 ** 18)/(IERC20(USDCs).totalSupply()));
        }
    ...
    function emergencyWithdraw() external{
        require(UserDeposit[msg.sender] != 0,"USDCVault: Nothing to withdraw");
    ...
    */
    ...
    function _withdraw(address user, uint256 _amount, address sendTo) nonReentrant _completePause internal _securityCheck(user,sendTo){
        ...
        require(_amount <= UserDeposit[user], "USDCvault: Invalid amount");
         if(rewards(user)[0] > 0){
            _claimRebaseReward(user);
        }else{
            ss[user] = currentReward;
        }
        IReceipt(USDCs).burn(address(this), _amount);
    ...
    function setRouter(address _router) external _isOperator {
        require(_router != address(0), "USDCVault: invalid data");
        router = _router;
    }

    function setSlippage(uint256 slippage_) external _isOperator {
        require(slippage_ > 0, "USDCVault: slippage can't be 0");
        slippage = slippage_;
    }
