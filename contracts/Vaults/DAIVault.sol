...
/**
@dev This contract is for DAI vault.
 */
contract DAIVault is Initializable, ReentrancyGuard, Pausable {
    ...
    address public DAIs; //Reciept token address
    address public DAI; // Address of DAI token
    ...
    function initialize(address _admin, address _DAI) external initializer {
        rewardRate = 10; // cofficient by 100
        slippage = 100; // cofficient by 100
        Admin = IAdmin(_admin);
        router = Admin.ApeswapRouter();
        DAI = _DAI;
        DAIs = Clones.clone(Admin.masterNTT());
        IReceipt(DAIs).initialize(_admin, address(this), "DAIs", "DAIs");
        epoch = 8 hours;
    }
    ...
    */
    ...
    function deposit(address user, uint256 amount, bool isBUSD) external nonReentrant _completePause whenNotPaused() _securityCheck(user,user) {
        require(amount > 0, "DAIVault: Amount must be greater than zero");
         if(rewards(user)[0] > 0){
            _claimRebaseReward(user);
        }else{
            ss[user] = currentReward;
        }
        IERC20(DAI).safeTransferFrom(user, address(this), amount);
        address[] memory path = new address[](2);
        path[0] = DAI;
        path[1] = Admin.BUSD();
        IERC20(DAI).safeApprove(router, amount);
        ...
        _convertBUSDToLP(
            ...
            Admin.USDy(),
            Admin.BUSD());
        IReceipt(DAIs).mint(address(this), amount);
    ...
    function withdraw(address user, uint256 amount, address sendTo) external whenNotPaused {
        require(user != address(0), "DAIvault: User address can not be empty");
        require(amount > 0, "DAIVault: Amount must be greater than zero");
    ...
    function distribute(uint256 _rebaseReward) external nonReentrant{
        ...
        if(IERC20(DAIs).totalSupply() != 0){
            currentReward = currentReward + ((_rebaseReward * 10 ** 18)/(IERC20(DAIs).totalSupply()));
    ...
    function emergencyWithdraw() external{
        require(UserDeposit[msg.sender] != 0,"DAIVault: Nothing to withdraw");
    ...
    */
    ...
    function _withdraw(address user, uint256 _amount, address sendTo) nonReentrant _completePause internal _securityCheck(user,sendTo){
        ...
        require(_amount <= UserDeposit[user], "DAIvault: Invalid amount");
         if(rewards(user)[0] > 0){
            _claimRebaseReward(user);
        }else{
            ss[user] = currentReward;
        }
        IReceipt(DAIs).burn(address(this), _amount);
        ...
        uint amountOut = _swap(result, path, sendTo);
        restrictTransfer[msg.sender] = block.number;
    }
    ...
    function setRouter(address _router) external _isOperator {
        require(_router != address(0), "DAIVault: invalid data");
        router = _router;
    }

    function setSlippage(uint256 slippage_) external _isOperator {
        require(slippage_ > 0, "DAIVault: slippage can't be 0");
        slippage = slippage_;
    }
